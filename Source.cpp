...
#define m 11         //Enter NO. LAYER OF TREATMENT
#define n 4             //Enter NO. OF REPLICATION
#define x 50
#define y 10
...
int main() {
    system("cls");
    //ifstream Agriculture;
    //Agriculture.open("test1.csv");
    //while (Agriculture.good()) {
      //  string line;
        //getline(Agriculture, line, ',');
       // cout << line << endl;



        float  sumt = 0.0, sumr = 0.0, mean;
        int i, j;

        float TR[m][n] = { {29.75 ,	20.81 ,	23.96 ,	23.16 },   //PASTE VALUE BETWEEN CURLY BRACES
    {23.12 ,	27.26 ,	24.16 ,	27.70 },                            //Format {{}};
    {33.74 ,	31.60 ,   38.48 ,	33.03 },
            //{ {t0r0 , t0r1, t0r2},          
            {25.82 ,	29.19 ,	26.47 ,	27.00 },                             //{t1r0 , t1r2, t1r3},
            {26.98 ,	25.19 ,	24.23 ,	24.57 },                             //{t2r0, t2r1, t2r3}
            {25.46 ,	25.81 ,	25.71 ,	21.54 },                             //                     };
            {22.57 ,	22.07 ,	27.73   ,  28.35 },
            {25.43 ,	25.16 ,	24.52 ,	28.61 },
            {20.34 ,	25.16 ,	25.90 ,	19.88 },
            {6.29 ,	7.52 ,	6.89 ,	7.47 },
            {22.98 ,	16.06 ,	17.69 ,	14.79}
        ...
        for (i = 0; i < m; ++i) {
            ...
            cout
                << "\n\tTotal of Treatment " << i << " = " << sumr << endl;


            sumr = 0;
        }

        float sqr2, sumofsqr2 = 0;
        for (i = 0; i < n; i++) {
            for (j = 0; j < m; j++) {
            ...
            cout
                << "\n\tTotal of Replication " << i << " = " << sumr << "  " << endl;
        ...
        for (i = 0; i < m; i++) {
            ...
            cout << "\n\tMean of Treatments " << i << " = " << mean << endl;
        ...
        cout << "\n---*---*----*-----*-----*------*-----*";
        dr = n - 1;
        cout << "\n\tDF of replication = " << dr << endl;
        dt = m - 1;
        cout << "\n\tDF of treatment = " << dt << endl;
        er = (m) * (n)-1 - (dt + dr);
        cout << "\n\tError of DF = " << er << endl;
        cout << "\n\t\t TOTAL = " << dr + dt + er << endl;
        ...
        cf2 = cf / (m * n);
        cout << "\nCF = " << cf;
        float ssofr, ssoft, ssot, ser, sqr3;
        int sqofele = 0, sumofsqele;
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                TR[i][j] = pow(TR[i][j], 2);
                sqofele += TR[i][j];
        ...
        ssofr = (sumofsqr2 / m) - (cf);
        ssoft = (sumofsqr / n) - (cf);
        ssot = sqofele - cf;
        ser = ssot - ssofr - ssoft;



        cout << "\n\t SS of replication is = " << ssofr << endl;
        cout << "\n\t SS of Treatment is = " << ssoft << endl;
        cout << "\n\t SS TOTAL = " << ssot << endl;
        cout << "\n\t  SS ERROR = " << ser << endl;

        float msofr, msoft, mser;
        msofr = ssofr / dr;
        msoft = ssoft / dt;
        mser = ser / (er);
        ...
        cout << "\n\tMS of replication is = " << msofr << endl;
        cout << "\n\tMS of Treatment is = " << msoft << endl;
        cout << "\n \tMS ERROR = " << (ser / er) << endl;
        ...
        cout << "\n\tFcal of Replication = " << fcalR << endl;
        cout << "\n\tFcal of Treatment = " << fcalT << endl;
        ...
        Fdist[x - 1][y - 1] = Fdist[dr][er];
        cout << "\n\tFtab 0.05 of REPLICATION is = " << Fdist[er - 1][dr - 1];
        ...
        Fdist[x - 1][y - 1] = Fdist[(dt)][er];
        cout << "\n\tFtab 0.05 of TREATMENT is = " << Fdist[er - 1][dt - 1];

        cout << "\n---*---*----*-----*-----*------*-----*";

        if (Fdist[er - 1][dr - 1] > fcalR) {
            cout << "\n\b\tTEST RESULT (REPLICATION) :- NOT SIGNIFICANT\b\n" << endl;
        }
        else
        {
            cout << "\n\b\tTEST RESULT (REPLICATION)  :-  SIGNIFICANT\b\n" << endl;
        }


        if (Fdist[er - 1][dt - 1] > fcalT) {

            cout << "\n\b\tTEST RESULT (TREATMENT) :- NOT SIGNIFICANT\b\n" << endl;
        }
        else
        {
            cout << "\n\b\tTEST RESULT (TREATMENT)  :-  SIGNIFICANT\b\n" << endl;
        }
